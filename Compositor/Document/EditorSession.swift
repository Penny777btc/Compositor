...
struct ImageLayer: Identifiable, Equatable {
    ...
    /// Set while this layer remains editable text; pixel edits clear it like they clear `shape`.
    var text: LayerText?
    /// Set while this layer remains an editable smooth gradient.
    var gradient: LayerGradient?
    /// Set while this layer remains an editable normalized vector path.
    var vectorPath: LayerVectorPath?
    /// Non-secret provenance for an image generated through an AI Provider.
    var generation: LayerGenerationRecord?
    ...
    func commitTransform() {
        ...
        if let group = edit.group {
            ...
            for (id, original) in group.originals {
                ...
                redrawGradient(at: index)
                redrawVectorPath(at: index)
            }
            endEdit()
            return
        }
