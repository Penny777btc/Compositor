...
final class ProjectWorkspace {
    ...
    func addTab(reuseEmpty: Bool = true) -> ProjectTab {
        ...
        let tab = ProjectTab(name: L10n.format("Untitled %lld", nextNumber))
        nextNumber += 1
    ...
    func copyLayer(_ id: UUID, into destination: UUID?, at point: CGPoint? = nil) async {
        ...
        guard used + added <= 100_000_000 else { target.session.importError = L10n.text("The copied layers exceed this project’s 100-megapixel limit."); return }
        isManaging = true
