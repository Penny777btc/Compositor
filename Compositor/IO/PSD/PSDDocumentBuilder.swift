...
nonisolated enum PSDDocumentBuilder {
    ...
    static func makeImport(_ document: PSDDocument, assets: [UUID: ImportedImage] = [:]) throws -> PSDImport {
        ...
        for record in document.layers {
            var notes: [String] = []
            if record.kind == .text {
                notes.append(L10n.text("Editable Photoshop text becomes pixels and can’t be retyped."))
            }
            if record.kind == .smartObject {
                notes.append(L10n.text("The smart object was rasterized. Linked contents can’t be edited."))
            }
            if record.kind == .effects {
                notes.append(L10n.text("Layer effects were discarded, so the appearance may differ."))
            }
            if record.kind == .vector {
                if record.shape != nil {
                    notes.append(contentsOf: record.shapeNotes)
                } else {
                    notes.append(L10n.text("Vector shape was rasterized to pixels."))
                }
            }
            if record.kind == .other {
                notes.append(L10n.text("This Photoshop layer type isn’t supported and was imported as pixels."))
            }
            if record.isGroup {
            ...
            if record.kind == .adjustment {
                if record.adjustment == nil {
                    notes.append(L10n.text("This adjustment type isn’t supported and was skipped."))
                } else {
                    notes.append(L10n.text("Adjustment parameters may not match Photoshop exactly."))
                }
            }
            for note in notes {
            ...
            if let maskImage = record.mask, let maskAsset = try? LayerMask.asset(from: maskImage) {
                ...
                conversions.append(PSDConversion(layerName: record.name, message: L10n.text("The layer mask couldn’t be converted to 8-bit grayscale and was skipped.")))
            }
            layers.append(layer)
        }
            ...
            if record.clipping {
                if let source = baseForParent[record.parentID],
                   ...
                   !sourceLayer.isGroup, sourceLayer.adjustment == nil {
                    ...
                    conversions.append(PSDConversion(layerName: record.name, message: L10n.text("This clipping mask’s base isn’t supported, so clipping was skipped.")))
                }
