...
nonisolated struct ProjectLayerRecord: Codable, Sendable {
    ...
    /// Editable normalized vector/hand-drawn path metadata.
    var vectorPath: LayerVectorPathStyle? = nil
    /// Non-secret provenance for an AI-generated raster. Credentials and reference bytes are never stored here.
    var generation: LayerGenerationRecord? = nil
}
...
nonisolated enum ProjectError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .version(let version): L10n.format("This project uses format version %lld. This app supports versions 1–11.", version)
...
actor ProjectStore {
    ...
    private func readPackage(_ url: URL) throws -> ProjectSnapshot {
        ...
        guard (1...11).contains(header.version) else { throw ProjectError.version(header.version) }
    ...
    private func validate(_ manifest: ProjectManifest) throws {
        ...
        guard (1...11).contains(manifest.version) else { throw ProjectError.version(manifest.version) }
        ...
        for layer in manifest.layers {
            ...
            if let generation = layer.generation {
                guard manifest.version >= 10, layer.isGroup != true, layer.adjustment == nil,
                      layer.shape == nil, layer.text == nil, layer.gradient == nil, layer.vectorPath == nil,
                      layer.imageFile == "\(layer.id.uuidString).png", generation.isValid else { throw ProjectError.invalid }
            }
            if let vectorPath = layer.vectorPath {
                guard manifest.version >= 11, layer.isGroup != true, layer.adjustment == nil,
                      layer.shape == nil, layer.text == nil, layer.gradient == nil, layer.generation == nil,
                      layer.imageFile == "\(layer.id.uuidString).png", vectorPath.isValid else { throw ProjectError.invalid }
            }
