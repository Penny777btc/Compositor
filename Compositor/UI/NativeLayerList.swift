...
private final class LayerCell: NSTableCellView, NSTextFieldDelegate {
    ...
    func configure(_ layer: ImageLayer, enabled: Bool, session: EditorSession, depth: Int, visible: Bool) {
        ...
        thumbnail.toolTip = L10n.text("Select image pixels")
        ...
        thumbnail.setAccessibilityLabel(L10n.format("Select image: %@", layer.name))
        ...
        dimensions.stringValue = layer.adjustment != nil ? L10n.text("Adjustment · Double-click to edit") : layer.isGroup ? L10n.text("Folder") : L10n.format("%lld × %lld px", Int(layer.size.width.rounded()), Int(layer.size.height.rounded()))
